...
over `/api`. This is a bare skeleton â€” no app features are implemented. Build on
top of it.
...
```
farm-ts/
  backend/   FastAPI + motor (async MongoDB) + Pydantic v2 â€” python, /root/.venv
...
frontend code always calls a **relative** path â€” `apiGet("/status")` â†’
`/api/status` â€” and never an absolute backend URL. The same code works in dev
...
- **Entry point**: `backend/server.py` â€” creates `app = FastAPI()`, creates
  ...
  added from `CORS_ORIGINS`. Never hang a route directly off `app` â€” it would
...
- **The route pattern** (copy `status` in `server.py`):
  ...
  runs â€” a malformed body never reaches your code, it gets an automatic `422`
  with a `{"detail": [...]}` body.
- **Growing the backend**: as `server.py` gets crowded, move models to
  ...
  prefix preserved).
- **MongoDB**: import the shared handle â€” `from lib.db import client, db`
...
- **Ids**: documents use a string `id` (`uuid4`) field, not Mongo's `ObjectId`
  â€” `ObjectId` is not JSON-serializable and leaks into response bodies. Keep the
  `uuid4` default-factory pattern from `StatusCheck`.
- **Config**: `backend/.env` â€” `MONGO_URL` (connection string), `DB_NAME`
...
- **Dates**: `backend/lib/dates.py` â€” `today_iso(tz=None)`. The pod clock is
...
- Tailwind CSS v4 (via the `@tailwindcss/vite` plugin â€” no separate
  `tailwind.config.js` needed) + shadcn/ui, initialized with the `base-nova`
  style and `neutral` base color, `@` path alias (`@/*` â†’ `src/*`) wired in both
  `tsconfig.app.json`/`tsconfig.json` and `vite.config.ts`.
- `react-router-dom` and `motion` are preinstalled â€” don't re-add them. `src/App.tsx`
  ...
  a `<Route>` for every page you write, in the same edit that creates the page â€” a
  page with no route is unreachable, and any URL without a matching `<Route>` renders a
  **blank page** â€” `<Routes>` matches nothing and mounts nothing.
...
- `src/lib/api.ts` â€” the typed fetch layer: `apiGet<T>`, `apiPost<T>`,
  `apiPut<T>`, `apiPatch<T>`, `apiDelete<T>`, all relative to base `/api`,
  throwing `ApiError` (with `status` and the parsed body) on any non-2xx.
  **Nothing infers across the Python boundary** â€” you declare the response type
...
- `src/pages/Home.tsx` is a minimal example of the wiring: TanStack Query's `useQuery`
  ...
  connectivity probe**, not a proof of the round trip â€” the result is deliberately
  ...
  static-preview rule in `TEMPLATE.md` Â§4 for why no page may be gated on a fetch.
...
â€” plain `tsc --noEmit` run from `frontend/` checks ZERO files (root tsconfig uses
...
through the core user journey. Clean on all three â†’ finish; any failure is a real
bug â€” fix it, re-run the failed check, and escalate to the testing subagent.
No routine typecheck/lint/smoke passes during the build â€” tier 1 runs exactly once.
...
**Backend (pytest)** â€” specs in `backend/tests/` as `test_*.py`, run with:
...
already parallel â€” do not pass your own `-n`) and `asyncio_mode = auto` (so
`async def test_...` needs no marker). Serial is `-n 0`, **never**
`-p no:xdist` (that errors, because `addopts` still passes `-n`/`--dist`).
`backend/tests/conftest.py` is pre-scaffolded â€” a sync `client` fixture
...
**Frontend (Playwright)** â€” `/app/tests/` is pre-scaffolded:
`playwright.config.ts` (canonical â€” edit the marked lines only),
...
`tests/e2e/`. Do NOT re-create the config/helpers or install/upgrade playwright â€”
...
This template runs under supervisord in the Emergent agent pod â€” supersedes any
